#!/usr/bin/env node
// Triage priority RA ingest candidates by meaningful content disposition.
//
...
const DEFAULT_PRIORITY_PREFIX = "P1";
...
function parseArgs(argv) {
  const args = {
    ...
    minFullChars: 500,
    priorityPrefix: DEFAULT_PRIORITY_PREFIX,
    outputPrefix: "",
  };
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    else if (arg === "--priority-prefix") args.priorityPrefix = argv[++i];
    else if (arg === "--output-prefix") args.outputPrefix = argv[++i];
  ...
  if (!args.priorityPrefix) throw new Error("--priority-prefix is required");
  if (!args.outputPrefix) args.outputPrefix = args.priorityPrefix.toLowerCase();
  return args;
}
...
Options:
  ...
  --priority-prefix <prefix>  Priority prefix to select. Default: ${DEFAULT_PRIORITY_PREFIX}
  --output-prefix <prefix>    Output filename/report prefix. Default: lowercase priority prefix.
  --out-dir <path>            Output directory. Default: reports/<output-prefix>-meaningful-triage-<timestamp>
...
function main() {
  ...
  const priorityLabel = args.priorityPrefix.toUpperCase();
  const outDir = args.outDir || path.join(DEFAULT_REPORTS, `${outputPrefix}-meaningful-triage-${timestamp()}`);
  ...
  const selected = priorityItems.filter((item) => String(item.priority || "").startsWith(args.priorityPrefix));
  ...
  const summary = {
    ...
    priorityPrefix: args.priorityPrefix,
    outputPrefix,
    selectedSources: selected.length,
    selectedBytes: p0Bytes,
    selectedBytesMB: Number((p0Bytes / 1048576).toFixed(1)),
    ...
    avoidedFullLlmCallPct: Number((((duplicateRows.length + lowRows.length + emptyRows.length) / Math.max(1, selected.length)) * 100).toFixed(1)),
    dispositionCounts,
    productCounts,
    workstreamCounts,
    pilot: {
  ...
  fs.writeFileSync(path.join(outDir, `${outputPrefix}-disposition-summary.json`), `${JSON.stringify(summary, null, 2)}\n`, "utf8");
  fs.writeFileSync(path.join(outDir, `${outputPrefix}-disposition-full.json`), `${JSON.stringify(dispositionRows, null, 2)}\n`, "utf8");
  fs.writeFileSync(path.join(outDir, `${outputPrefix}-canonical-groups.json`), `${JSON.stringify(canonicalGroups, null, 2)}\n`, "utf8");
  fs.writeFileSync(path.join(outDir, `${outputPrefix}-pilot-full-wiki.json`), `${JSON.stringify(pilot, null, 2)}\n`, "utf8");

  writeCsv(path.join(outDir, `${outputPrefix}-disposition-full.csv`), dispositionRows, [
    "disposition",
    "dispositionReason",
    "queueId",
    "priority",
  ...
  writeCsv(path.join(outDir, `${outputPrefix}-pilot-full-wiki.csv`), pilot, [
    "pilotRank",
    "queueId",
    "priority",
    "score",
  ...
  const report = [
    `# ${priorityLabel} Meaningful Content Triage`,
    "",
    ...
    "## Summary",
    "",
    `- ${priorityLabel} sources: ${summary.selectedSources.toLocaleString()}`,
    `- ${priorityLabel} bytes: ${summary.selectedBytesMB.toLocaleString()} MB`,
    ...
    "## Output Files",
    "",
    `- \`${outputPrefix}-disposition-summary.json\``,
    `- \`${outputPrefix}-disposition-full.json\``,
    `- \`${outputPrefix}-disposition-full.csv\``,
    `- \`${outputPrefix}-canonical-groups.json\``,
    `- \`${outputPrefix}-pilot-full-wiki.json\``,
    `- \`${outputPrefix}-pilot-full-wiki.csv\``,
    "",
  ].join("\n");
  fs.writeFileSync(path.join(outDir, `${outputPrefix}-meaningful-triage-report.md`), report, "utf8");
