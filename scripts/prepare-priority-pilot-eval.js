#!/usr/bin/env node
// Prepare a small priority representative-source evaluation bundle.
//
...
const DEFAULT_TRIAGE_DIR = "reports\\p1-meaningful-triage-202606241713";
const DEFAULT_COUNT = 30;
const DEFAULT_START_RANK = 1;
const DEFAULT_OUTPUT_PREFIX = "p1";

let globalOutputPrefix = DEFAULT_OUTPUT_PREFIX;
let globalLabel = DEFAULT_OUTPUT_PREFIX.toUpperCase();
...
function parseArgs(argv) {
  ...
  if (!args.outputPrefix) throw new Error("--output-prefix is required");
  if (!args.label) args.label = args.outputPrefix.toUpperCase();
  return args;
}
...
function printHelp() {
  console.log(`
Usage:
  node scripts/prepare-priority-pilot-eval.js [options]
...
Options:
  ...
  --source <pilot|full>    Use <prefix>-pilot-full-wiki.json or all full_wiki_candidate rows. Default: pilot
  --output-prefix <prefix> Output file prefix. Default: ${DEFAULT_OUTPUT_PREFIX}
  --label <label>          Human label used in rubric/report. Default: uppercase output prefix.
  --out-dir <path>         Output directory. Default: reports/<prefix>-pilot-eval-<timestamp>
`);
}
...
function fullRepresentativeCandidates(triageDir) {
  const dispositionFile = path.join(triageDir, `${globalOutputPrefix}-disposition-full.json`);
...
function pilotCandidates(triageDir) {
  const pilotFile = path.join(triageDir, `${globalOutputPrefix}-pilot-full-wiki.json`);
...
function main() {
  ...
  globalOutputPrefix = args.outputPrefix;
  globalLabel = args.label;
  const outDir = args.outDir || path.join("reports", `${args.outputPrefix}-pilot-eval-${timestamp()}`);
